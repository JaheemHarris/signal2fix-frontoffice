    ...
    >
      <Popup>
        <Card sx={{ maxWidth: 200 }}>
          ...
          <CardActions>
            <Button size="small" href={"/fiche/" + signalement.idSignalement}>
              Voir
            </Button>
          </CardActions>
        </Card>
...
function GeneralContent() {
  ...
  const handleSearch = (event) => {
    ...
    for(let type of types){
      let reste = type.id-parseInt(idType);
      if(reste===0){
        setTypeText(type.type)
        break;
      }
    }
    console.log("type"+typeText);
    if (idType === "") setIdType(null);
    if (idStatus === "") setIdStatus(null);
    ...
    <Grid>
      <Grid className="searchBox">
        ...
        <form onSubmit={handleSearch}>
          <Grid container>
            <Grid item md={2.5} style={{ paddingLeft: "4%" }}>
      ...
      <Grid container style={{ paddingTop: "0.5%" }}>
        <Grid item md={4} sm={4} xs={4} style={{ paddingLeft: "2%" }}>
          ...
          <p>Date : Tous</p>
          <br />
          <p>Nombres: {signalements.length} signalements</p>
        </Grid>
        <Grid item md={8} sm={8} xs={8} lg={8} style={{ paddingRight: "3%" }}>
          <Carte signalements={signalements}></Carte>
        </Grid>
      </Grid>
    </Grid>
  );
